...
def calc_perwise_differences(sr):
    """
    Calculate differences between all pairs of items in a pnadas series
    """
    return sr.to_frame().join(sr.to_frame().T, lsuffix='_l', rsuffix='_r').dropna().query('index_l != index_r').assign(
        diff=lambda x: x[0] - x[1]).drop([0], axis=1)
